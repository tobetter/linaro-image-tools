...
class file():
    def __init__( self ):
        self.homedir = os.path.expanduser( "~" ) + os.sep + ".fetch_image"
        self.cachedir = self.homedir + os.sep + "cache" + os.sep

    def create_media( self, image_url_, hwpack_url_, settings_ ):
        command = "linaro-media-create"

        binary_file = self.download( image_url_ )
        hwpack_file = self.download( hwpack_url_ )

        print binary_file, hwpack_file

        if( settings_.has_key('rootfs') and settings_['rootfs'] ):
            command += " --rootfs " + settings_['rootfs']
        ...
        if( settings_.has_key('mmc') and settings_['mmc'] ):
            command += " --mmc " + settings_['mmc']
            
        if( settings_.has_key('image_file') and settings_['image_file'] ):
            command += " --image_file " + settings_['image_file']

        if( settings_.has_key('image_size') and settings_['image_size'] ):
            command += " --image_size " + settings_['image_size']

        if( settings_.has_key('swap_file') and settings_['swap_file'] ):
            command += " --swap_file " + settings_['swap_file']

        command += ( " --dev " + settings_['hardware']
                     + " --binary " + binary_file
                     + " --hwpack " + hwpack_file )
    ...
    def download( self, url_, force_download_ = False, quiet_ = False ):
        """Downloads the file requested buy URL to the local cache and returns the full path to the downloaded file"""

        url_search = re.search( r"^http://(.*)/(.*?)$", url_ )
        assert url_search, "URL in unexpectd format"
...
class config():
    def read_config( self, file_name_ ):
        ...
        try:
            ...
            print >> sys.stderr, "Unable to read settings file", file_name_
            sys.exit(0)
    ...
    def parse_args( self, args_ ):
        ...
        parser.add_argument( "--swap-file", help = "Swap file size to pass to linaro-media-create" )
        parser.add_argument( "--image-size", help = "Image file size to pass to linaro-media-create" )
        parser.add_argument( "--rootfs", help = "Root file system type to pass to linaro-media-create" )
...
class db():
    ...
    def record_url( self, url_, table_ ):
        ...
        base_url, url_validator, url_chunk_names = self.url_parse[table_]

        assert base_url != None, "Can not match the URL received (" + url_ + ") to an entry provided by add_url_parse_list"
        assert re.search( '^' + base_url, url_ )


        if( not re.search( url_validator, url_ ) ): #Make sure that the URL matches the validator
            return
        ...
        url_match = re.search( base_url + r"(.*)$", url_ )
    ...
    def get_url( self, table_, key_value_pairs_, sql_extras_ = None ):
        ...
        if( self.c.fetchone() != None ):
            print >> sys.stderr, "Warning: Matched more than one URL in database. Using first found"
                ...
                if( args_['build'] == "latest" ):
                    image_url = self.get_url( "snapshot_binaries",
                                    [
                                     ( "image", args_['image'] ),
                                     ( "date", "latest" )],
                                    "ORDER BY build DESC" ) # First result is latest build
