...
class FileHandler():
    ...
    class DummyEventHandler():
        """Just a sink for events if no event handler is provided to create_media"""
    ...
    def create_media(self, image_url, hwpack_url, settings, run_in_gui = False, event_handler = None):
        """Create a command line for linaro-media-create based on the settings provided
        then run linaro-media-create, either in a separate thread (GUI mode) or in the current
        one (CLI mode)."""
        import subprocess
        ...
        args = []
        args.append("pkexec")
        args.append(os.path.dirname(os.path.abspath(sys.argv[0])) + os.sep + "linaro-media-create")
        args.append("--yes-to-mmc-selection")
               
        event_handler.event_start("download OS")
        try:
            binary_file = self.download(image_url, settings["force_download"], show_wx_progress = run_in_gui, wx_progress_title = "Downloading file 1 of 2")
        except:
        ...
        try:
            hwpack_file = self.download(hwpack_url, settings["force_download"], show_wx_progress = run_in_gui, wx_progress_title = "Downloading file 2 of 2")
        except:
            # Something went wrong with downloading. Hardly matters what, we can't continue.
            print "Unexpected error:", sys.exc_info()[0]
        ...
        if settings.has_key('rootfs') and settings['rootfs']:
            args.append("--rootfs")
            args.append(settings['rootfs'])

        assert bool(settings.has_key('image_file') and settings['image_file']) ^ bool(settings.has_key('mmc') and settings['mmc']), "Please specify either an image file, or an mmc target, not both."

        if settings.has_key('mmc') and settings['mmc']:
            args.append("--mmc")
            args.append(settings['mmc'])

        if settings.has_key('image_file') and settings['image_file']:
            args.append("--image_file")
            args.append(settings['image_file'])

        if settings.has_key('image_size') and settings['image_size']:
            args.append("--image_size")
            args.append(settings['image_size'])

        if settings.has_key('swap_file') and settings['swap_file']:
            args.append("--swap_file")
            args.append(settings['swap_file'])

        if settings.has_key('yes_to_mmc_selection') and settings['yes_to_mmc_selection']:
            args.append("--yes-to-mmc_selection")
        ...
        if run_in_gui:
            ...
            return # We step from the GUI thread, first the GUI calls start_lmc_gui_thread, then process_lmc_gui_thread...
    ...
    class LinaroMediaCreate(threading.Thread):
        ...
        def run(self):
            ...
            1. Tell us that an event has happened that we can use to update the UI progress.
            2. Tell us that linaro-media-create is asking a question that needs to be
               re-directed to the GUI"""
            import subprocess
            self.create_process = subprocess.Popen(self.lmcargs, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            ...
            while(1):
                if self.create_process.poll() != None:  # linaro-media-create has finished.
                    self.event_queue.put(("terminate")) # Tell the GUI
                    return                              # Terminate the thread
                
                self.input = self.create_process.stdout.read(1)
                
                # We build up lines buy extracting data from linaro-media-create a single character at
                # a time. This is because if we fetch whole lines, using Popen.communicate a character
                # is automatically sent back to the running process, which if the process is waiting
                # for user input, can trigger a default action. By using stdout.read(1) we pick off
                # a character at a time and avoid this problem.
                if self.input == '\n':
                    if self.save_lines:
                ...
                elif self.line == "WARNING: The following packages cannot be authenticated!":
                    self.saved_lines = ""
                    self.save_lines = True
                    
                elif self.line == "Install these packages without verification [y/N]? ":
                    self.saved_lines = re.sub("WARNING: The following packages cannot be authenticated!", "", self.saved_lines)
                    self.event_queue.put(("start", "unverified_packages:" + self.saved_lines))
                    self.line = ""
                ...
                elif self.line == "Created:" and self.state == "create file system":
    ...
    def start_lmc_gui_thread(self, event_queue):
        self.lmc_thread = self.LinaroMediaCreate(self.event_handler, self.lmcargs, event_queue)
        self.lmc_thread.start()

    def kill_create_media(self):
        pass # TODO: Something! Need to make sure all child processes are terminated.
    
    def send_to_create_process(self, text):
        self.lmc_thread.send_to_create_process(text)

    def name_and_path_from_url(self, url):
        """Return the file name and the path at which the file will be stored on the local
        system based on the URL we are downloading from"""
    ...
    def download(self, url, force_download = False, show_wx_progress = False, wx_progress_title = None):
        ...
        if(force_download != True and os.path.exists(file_name)):
            print file_name + " already cached. Not downloading (use --force-download to override)."
        ...
        if show_progress:
            ...
            if show_wx_progress:
                ...
                self.create_wx_progress(wx_progress_title, "Downloading " + just_file_name)
            else:
                print "Fetching", url
        else:
        ...
        if show_progress and show_wx_progress:
            self.update_wx_progress(0) # Just update the download box before we get the first %
            ...
            if(len(chunk)):
                ...
                if show_progress:
                    if show_wx_progress:
                        ...
                        sys.stdout.write("\r%d%%" % (chunks_downloaded/10)) # Have 1000 chunks
    ...
    def download_if_old(self, url, force_download, show_wx_progress = False):
        ...
        if(not os.path.isdir(file_path)):
            os.makedirs(file_path)
        try:
            force_download = force_download == True or (time.mktime(time.localtime()) - os.path.getmtime(file_path_and_name) > 60 * 60 * 24)
        except OSError:
            force_download = True # File not found...
        
        return self.download(url, force_download, show_wx_progress)
    
    def update_files_from_server(self, force_download = False, show_wx_progress = False):
        
        settings_url      = "http://z.nanosheep.org/fetch_image_settings.yaml"
        server_index_url  = "http://z.nanosheep.org/server_index.bz2"
        
        self.settings_file = self.download_if_old(settings_url,     force_download, show_wx_progress)
        self.index_file    = self.download_if_old(server_index_url, force_download, show_wx_progress)
...
class Config():
    """Reads settings from the settings YAML file as well as those from the command
    ...
    def read_config(self, file_name):
        try:
            ...
            if(f):
                self.settings = dict(self.settings.items() + yaml.load(f.read()).items())
        ...
        # At this point, assume settings have been loaded. We need some reverse mappings set up
...
class DB():
    ...
    def record_url(self, url, table):
        ...
        assert self.url_parse[table]["base_url"] != None, "Can not match the URL received (" + url + ") to an entry provided by add_url_parse_list"
        assert re.search('^' + self.url_parse[table]["base_url"], url)

        if(not re.search(self.url_parse[table]["url_validator"], url)): #Make sure that the URL matches the validator
            return

        print url

        assert url not in self.touched_urls, "URLs expected to only be added to 1 place\n" + url

        self.touched_urls[url] = 1
        ...
        url_match = re.search(self.url_parse[table]["base_url"] + r"(.*)$", url)
        ...
        sqlcmd += "".join([ "?, " for x in range(length + 1) ]) # +1 is so we have room for url
    ...
    def get_url(self, table, key_value_pairs, sql_extras = None):
        """Return the first matching URL from the specified table building up a SQL query based on the
        values inkey_value_pairs creating key == value sections of the query. sql_extras is appended
        to the created SQL query, so we can get the first entry in an ordered list for instance"""
        self.c = self.database.cursor()
    ...
    def get_builds(self, platform, image = None):
        """return a complete list of builds available from the releases repository"""
        build_tuples        = self.execute_return_list('select distinct build from release_binaries where platform == "' + platform + '"' )
        hwpack_build_tuples = self.execute_return_list('select distinct build from release_hwpacks  where platform == "' + platform + '"' )
        
        # Need to also check hwpacks and image -> some builds are only available for a particular image (OS).
        # Guess this makes sense as OS customisations may come and go.
        ...
        for build in image_builds:
            if build in hwpack_builds:
                builds.append(build) # Only use a build if it exists for the hardware pack as well as the image and platform chosen
    ...
    def get_hwpacks(self, table, platform = None):
        """Return a list of all the hardware packs available in the specified table"""
    ...
    def get_dates_and_builds_of_snapshots(self):
        ...
        self.execute("select distinct date from snapshot_hwpacks order by date")
        ...
        self.execute("select distinct date from snapshot_binaries order by date")
        ...
        # Now anything with both a hwpack and a file system has a value of 2 set
        date_with_build = {}
    ...
    def get_hardware_from_db(self, table):
        ...
        self.execute("select distinct hardware from " + table + " order by hardware")
    ...
    def image_hardware_combo_available(self, release_or_snapshot, image, hwpacks):
        """Try and find a matching plaform, build pair for both the provided image and one of the hwpacks in the list."""
        binary_table = release_or_snapshot + "_binaries"
        hwpack_table = release_or_snapshot + "_hwpacks"
        
        binary_list = self.execute_return_list('select distinct platform, build from ' + binary_table + ' where image == ?', (image,))
        
        for hwpack in hwpacks:
            hwpack_list = self.execute_return_list('select distinct platform, build from ' + hwpack_table + ' where hardware == ?', (hwpack,))
    ...
    def hardware_is_available_in_table(self, table, hardware):
        return len(self.execute_return_list('select url from ' + table + ' where hardware == ?', (hardware,))) > 0

    def hardware_is_available_for_platform(self, hardware_list, platform, table = "release_hwpacks"):
        for hardware in self.execute_return_list('select distinct hardware from ' + table + ' where platform == ?', (platform,)):
            if hardware[0] in hardware_list:
                return True
            
        return False
    
    def get_available_hwpacks_for_hardware_build_plaform(self, hardware_list, platform, build):
        hwpacks = []
        for hardware in self.execute_return_list('select distinct hardware from release_hwpacks where platform == ? and build == ?',
                                                    (platform, build)):
            if hardware[0] in hardware_list:
    ...
    def image_is_available_for_platform(self, table, platform, image):
        return len(self.execute_return_list('select * from ' + table + ' where platform == ? and image == ?',
                                                    (platform, image))) > 0

    def hardware_is_available_for_platform_build(self, hardware_list, platform, build):
        for hardware in self.execute_return_list('select distinct hardware from release_hwpacks where platform == ? and build == ?',
                                                    (platform, build)):
            if hardware[0] in hardware_list:
                return True
    ...
    def build_is_available_for_platform_image(self, table, platform, image, build):
        return len(self.execute_return_list('select * from ' + table + ' where platform == ? and image == ? and build == ?',
                                                    (platform, image, build))) > 0
        

    def get_available_hwpacks_for_hardware_snapshot_build(self, hardware_list, platform, date, build):
        hwpacks = []
        for hardware in self.execute_return_list('select distinct hardware from snapshot_hwpacks where platform == ? and date == ? and build == ?',
                                                    (platform, date, build)):
            if hardware[0] in hardware_list:
                hwpacks.append(hardware[0])
    ...
    def get_binary_builds_on_day_from_db(self, image, date, hwpacks):
        """Return a list of build numbers that are available on the given date for the given image ensuring
        that the reqiested hardware is also available."""
        
        # Remove the dashes from the date we get in (YYYY-MM-DD --> YYYYMMDD)
        date = re.sub('-', '', date)
        
        binaries = self.execute_return_list("select build from snapshot_binaries where date == ? and image == ?", (date, image))
        
        if len(binaries) == 0:
            # Try adding "linaro-" to the beginning of the image name
            binaries = self.execute_return_list("select build from snapshot_binaries where date == ? and image == ?", (date, "linaro-" + image))
        
        for hwpack in hwpacks:
            builds = self.execute_return_list("select build from snapshot_hwpacks  where date == ? and hardware == ?", (date, hwpack))
        
            if len(builds): # A hardware pack exists for that date, return what we found for binaries
                return binaries
    ...
    def get_next_prev_day_with_builds(self, image, date, hwpacks):
        """Searches forwards and backwards in the database for a date with a build. Will look 1 year in each direction. Returns a tuple of dates in YYYYMMDD format."""
        
        # Split ISO date into year, month, day
        date_chunks = date.split('-')
        
        current_date = datetime.date(int(date_chunks[0]), int(date_chunks[1]), int(date_chunks[2]))
        one_day = datetime.timedelta(days=1)
        future_date = None
        past_date = None
        
        day_count = 0
        while(1): # Look in the future from the given date until we find a day with a build on it
            current_date += one_day
            
            builds = []
            for hwpack in hwpacks:
                builds = self.get_binary_builds_on_day_from_db(image, current_date.isoformat(), [hwpack])
                if len(builds):
                    break
            
            if len(builds):
                # Found a date with >0 builds
                future_date = current_date.isoformat()
                break
        ...
        day_count = 0
        current_date = datetime.date(int(date_chunks[0]), int(date_chunks[1]), int(date_chunks[2]))
        while(1): # Look into the past from the given date until we find a day with a build on it
            current_date -= one_day
            ...
            if len(builds):#
                # Found a date with >0 builds
                past_date = current_date.isoformat()
                break
    ...
    def get_os_list_from(self, table):
        return [item[0] for item in self.execute_return_list("select distinct image from " + table)]
    ...
    def get_image_and_hwpack_urls(self, args):
        ...
        if(args['release_or_snapshot'] == "snapshot"):
            ...
            #print "       ", args['image'], args['build'], args['platform']
